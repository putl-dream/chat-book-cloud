...
_最后更新时间: 2026-02-12 (基于当前代码库验证)_
...
### 1.1 DO/VO 定义严重不一致 [已修复]

- **状态**: **已修复**
  - 经核查，`ArticleVO` 与 `ArticleDO` 字段已对齐，`ArticleVO` 包含了 `cover`, `category`, `abstractText`, `status` 等字段。
- **问题描述**:
...
### 1.2 跨服务数据对象类型不匹配 [部分修复]

- **状态**: **部分修复**
  - 字段类型已一致（均为 Integer），但 User 服务中的 `UserFootVO` 缺少无参构造器，可能导致序列化问题。
...
### 4.1 AI 助手 (AI Assistant) - 完全缺失 [未修复]
...
### 4.2 消息通知 (Message Center) - 逻辑错误 [未修复]
...
1.  **Phase 1 (Fix Bugs)**: 修正 DO/VO 不一致（已完成），统一跨服务对象类型（部分完成），提取配置消除硬编码（待处理）。
2.  **Phase 2 (Standardization)**: **(新增)** 统一异常处理、响应结构、日志规范；添加事务注解。
3.  **Phase 3 (Gap Filling)**: 重构消息中心逻辑，实现真正的通知列表；补全 AI 助手后端。
4.  **Phase 4 (Architecture)**: 改造文件上传为 OSS（已完成），独立公共依赖模块。
