  ...
  <section class="page-shell">
    <template v-if="reviewPage">
      <div class="page-hero compact">
        ...
        <h1>文章审核工作台</h1>
        <p class="hero-copy">
          Vite + Vue 版本保留了待审核列表、通过、驳回和批量处理流程，并把原先依赖 Next.js
          App Router 的交互全部迁到了浏览器端。
        </p>
      </div>
      ...
      <section v-if="focusedArticle" class="panel">
        <div class="panel-header">
          <div>
            <p class="section-kicker">Detail Placeholder</p>
            <h3>文章详情入口预留</h3>
          </div>
        ...
        <div class="stack-list">
          <article class="stack-item">
            <div class="stack-title-row">
              ...
              <span class="pill pill-warn">待补详情接口</span>
            </div>
            <p>当前列表与审核动作已经可用，后续建议继续补齐标签、正文详情和审核记录字段。</p>
          </article>
        </div>
      </section>
      ...
      <div class="content-grid two-column">
        <section class="panel">
          ...
          <RequestStatePanel
            ...
            title="当前没有待审核文章"
            description="审核队列为空，后续可继续补齐详情字段与审核记录查询。"
          />
          ...
          <div v-else class="stack-list">
            ...
            <article v-for="article in reviewPage.list" :key="article.id" :id="`review-${article.id}`" class="review-card">
              ...
              <div class="chip-row">
                <span class="chip">标签字段待接口补齐</span>
                <span class="chip">内容类型待接口补齐</span>
              ...
              <div class="review-card-actions">
                ...
                <div class="inline-actions">
                  ...
                  <button class="table-action-button" type="button" @click="focusArticle(article.id)">查看详情占位</button>
                </div>
              </div>
        ...
        <section class="panel">
          ...
          <div class="stack-list">
            <article class="stack-item">
              <div class="stack-title-row">
                <h4>内容预审</h4>
                <span class="pill pill-safe">已接通真实列表</span>
              </div>
              <p>当前已展示标题、作者、分类、时间和互动统计。</p>
            </article>
            <article class="stack-item">
              <div class="stack-title-row">
                <h4>审核结论提交</h4>
                <span class="pill pill-safe">基础闭环可用</span>
              </div>
              <p>管理员可以直接提交通过、驳回和批量处理，并记录驳回原因。</p>
            </article>
            <article class="stack-item">
              <div class="stack-title-row">
                <h4>详情字段补齐</h4>
                <span class="pill pill-warn">仍有缺口</span>
              </div>
              <p>标签、内容类型和正文详情仍待后端返回，页面已保留占位。</p>
            </article>
          </div>
        </section>
      </div>
...
import { articleCategoryMap } from "@/data/admin-config";
import {
...
async function loadReviewPage() {
  try {
    errorMessage.value = "";
    reviewPage.value = await getReviewArticlesPage({ page: page.value, size: size.value });
    selectedIds.value = [];
...
async function handleBatchApprove() {
  ...
  if (!window.confirm(`确认批量通过选中的 ${selectedIds.value.length} 篇文章吗？`)) {
    return;
  }
...
watch([page, size], loadReviewPage, { immediate: true });
</script>
