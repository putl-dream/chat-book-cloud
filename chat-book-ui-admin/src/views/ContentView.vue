  ...
  <section class="page-shell">
    ...
    <section class="panel">
      ...
      <div class="toolbar-grid">
        <label class="field">
          <span>关键词</span>
          <input v-model="filter.keyword" placeholder="搜索标题或摘要" @keydown.enter="applyFilters" />
        </label>
        <label class="field">
          <span>状态</span>
      ...
      <section class="panel">
        ...
        <div class="review-bulk-bar">
          <label class="review-select-toggle">
            <input :checked="allSelected" :disabled="submitting || !articlePage.list.length" type="checkbox" @change="toggleAllSelection" />
            <span>当前页全选</span>
          </label>
          <div class="inline-actions">
            <span class="pill pill-neutral">已选 {{ selectedIds.length }} 篇</span>
            <button class="panel-action-button" :disabled="submitting || selectedIds.length === 0" type="button" @click="selectedIds = []">清空选择</button>
            <button class="panel-action-button primary" :disabled="submitting || selectedIds.length === 0" type="button" @click="handleBatchPublish">批量发布</button>
            <button class="panel-action-button" :disabled="submitting || selectedIds.length === 0" type="button" @click="handleBatchUnpublish">批量下架</button>
            <button class="panel-action-button danger" :disabled="submitting || selectedIds.length === 0" type="button" @click="handleBatchDelete">批量删除</button>
          </div>
        </div>
          ...
          <table class="data-table">
            ...
            <tbody>
              <tr v-for="article in articlePage.list" :key="article.id">
                ...
                <td>{{ articleCategoryMap[article.category] || "未分类" }} / {{ contentTypeMap[article.contentType] || "未分类" }}</td>
                <td>
                  <span
                    :class="[
                      'pill',
...
const publishedCount = computed(() => articlePage.value?.list.filter((a) => a.status === 2).length ?? 0);
const draftCount = computed(() => articlePage.value?.list.filter((a) => a.status === 0).length ?? 0);
const deletedCount = computed(() => articlePage.value?.list.filter((a) => a.status === -1).length ?? 0);
const allSelected = computed(
  () => !!articlePage.value?.list.length && articlePage.value.list.every((a) => selectedIds.value.includes(a.id))
);
...
async function runAction(fn: (id: number) => Promise<void>, id: number, label: string) {
  try {
    ...
    errorMessage.value = error instanceof BrowserApiError ? error.message : `${label}操作失败，请稍后重试。`;
  } finally {
...
async function runBatchAction(fn: (ids: number[]) => Promise<void>, ids: number[], label: string) {
  try {
    ...
    errorMessage.value = error instanceof BrowserApiError ? error.message : `批量${label}失败，请稍后重试。`;
  } finally {
    submitting.value = false;
    setTimeout(() => { message.value = ""; }, 3000);
  }
...
async function handlePublish(id: number) {
  if (!window.confirm(`确认发布文章 #${id} 吗？`)) return;
  await runAction(publishArticle, id, "发布");
}

async function handleUnpublish(id: number) {
  if (!window.confirm(`确认下架文章 #${id} 吗？`)) return;
  await runAction(unpublishArticle, id, "下架");
}

async function handleDelete(id: number) {
  if (!window.confirm(`确认删除文章 #${id} 吗？此操作不可恢复。`)) return;
  await runAction(deleteArticle, id, "删除");
}

async function handleRestore(id: number) {
  if (!window.confirm(`确认恢复文章 #${id} 吗？`)) return;
  await runAction(restoreArticle, id, "恢复");
}

async function handleBatchPublish() {
  if (selectedIds.value.length === 0) { errorMessage.value = "请先选择要批量发布的文章。"; return; }
  if (!window.confirm(`确认批量发布选中的 ${selectedIds.value.length} 篇文章吗？`)) return;
  await runBatchAction(batchPublish, [...selectedIds.value], "发布");
}

async function handleBatchUnpublish() {
  if (selectedIds.value.length === 0) { errorMessage.value = "请先选择要批量下架的文章。"; return; }
  if (!window.confirm(`确认批量下架选中的 ${selectedIds.value.length} 篇文章吗？`)) return;
  await runBatchAction(batchUnpublish, [...selectedIds.value], "下架");
}

async function handleBatchDelete() {
  if (selectedIds.value.length === 0) { errorMessage.value = "请先选择要批量删除的文章。"; return; }
  if (!window.confirm(`确认批量删除选中的 ${selectedIds.value.length} 篇文章吗？此操作不可恢复。`)) return;
